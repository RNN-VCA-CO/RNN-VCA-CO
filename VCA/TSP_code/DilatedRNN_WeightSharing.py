...
Here, we define the Dilated RNNwavefunction class with weight sharing, which contains the sample method
...
class DilatedRNNWavefunction_WeightSharing(object):
    def __init__(self,systemsize,cell=tf.nn.rnn_cell.BasicRNNCell,activation=tf.nn.relu,units=[2],scope='DilatedRNNwavefunction', seed = 111):
        ...
        with self.graph.as_default():
            with tf.variable_scope(self.scope,reuse=tf.AUTO_REUSE):
                ...
                self.dense = tf.layers.Dense(self.N,activation=tf.nn.softmax,name='DRNN_dense') #Define the Fully-Connected layer followed by a Softmax
